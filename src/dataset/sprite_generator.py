...
class SpriteGenerator:
    def __init__(
        self,
        # shape selection
        shape_indices: Optional[Sequence[int]] = None,
        # image properties
        height: int = 60,
        ...
        interpolation = Image.LANCZOS,  # type: ignore
        canonical_interpolation = Image.NEAREST,  # type: ignore
    ):
        if width is None:
            width = height
        if bg_color is None:
        ...
        self.num_bg_colors = num_bg_colors
    ...
    def __call__(self, sprite_dict: SpriteDict, folder: str | Path) -> None:
        folder = Path(folder)
        ...
        if self.shape_indices is not None:
            shapes = np.array(self.shape_indices)
        else:
        ...
        lim_bg_colors = (0, 1 - 1 / self.num_bg_colors)
        bg_colors = np.linspace(*lim_bg_colors, self.num_bg_colors, endpoint=True)
        ...
        all_combinations = product(
            shapes, colors, scales, angles, pos_xs, pos_ys, bg_colors
        )

        if self.num_colors > 1 and self.num_bg_colors > 1:
            all_combinations = [c for c in all_combinations if c[1] != c[6]]
        else:
        ...
        json_output = {
            ...
            'meta': {
                ...
                'factors': [
                    "shape", "color", "scale", "angle", "pos_x", "pos_y", "bg_color"
                ],
                'shape_names': {i.value: i.name for i in type(sprite_dict).Names if i.value in shapes},
                'unique_values': {
    ...
    def generate_image(self, shape_dict, shape, color, scale, angle, tx, ty, bg_color=0.0, value=1.0):
        ...
        sprite = ShapePath(sprite.vertices - pixel_correction_mask)
        ...
        # Use default background color if only value is used, normally black.
        if self.num_bg_colors == 1:
            bg_rgb = self.bg_color
        else:
            bg_rgb = hsv_to_rgb(bg_color, 1.0, 1.0)
        ...
        if self.num_colors == 1:
            ...
            color = hsv_to_rgb(color, 1.0, value)
        ...
        # pad image
        image = Image.new('RGB', self.image_size, color=bg_rgb)
        image.paste(canvas, self.padding)

        return image

    def generate_instance(self, params):
        i, (shp, col, scl, ang, tx, ty, bg) = params

        image_file = f"image_{i:08}.png"

        image = self.generate_image(self._sprite_dict, shp, col, scl, ang, tx, ty, bg)
        assert self._save_folder is not None
        image.save(self._save_folder / image_file)
...
def hsv_to_rgb(h, s=1.0, v=1.0):
    ...
    Args:
        h: Hue in range [0, 1]
        s: Saturation in range [0, 1], default 1.0
        v: Value in range [0, 1], default 1.0

    Returns:
        Tuple of (R, G, B) values in range [0, 255]
    """
    return tuple((255 * np.array(colorsys.hsv_to_rgb(h, s, v))).astype(np.uint8))

...
from .spatial import PositionEmbedding1D, PositionEmbedding2D
...
class SpatialTokenDict(nn.Module):
    def __init__(self, vocab_size, embedding_dim, height, width=None, dropout=0.0):
        super().__init__()
        self.embedding_dict = nn.Embedding(vocab_size, embedding_dim)
        self.add_pos_emb = PositionEmbedding2D(embedding_dim, height, width, 'cardinal')
    ...
    def forward(self, index_weights, pos=None):
        index = index_weights.argmax(dim=-1)
        embeddings = self.embedding_dict(index)
        return self.dropout(self.add_pos_emb(embeddings, pos))
...
class TokenDict(nn.Module):
    def __init__(self, vocab_size, embedding_dim, max_seq_len, dropout=0.0):
        super().__init__()
        self.embedding_dict = nn.Embedding(vocab_size, embedding_dim)
